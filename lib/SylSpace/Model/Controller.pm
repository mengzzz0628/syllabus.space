...
use base 'Exporter';
@ISA = qw(Exporter);
our @EXPORT =qw(timedelta standard global_redirect timezones btn mkdatatable);  ## very common
our @EXPORT_OK =qw( carefreebutton msghash2string drawform domain epochof epochtwo btnsubmit btnblock btnxs drawmore ifilehash2table fileuploadform displaylog global_redirectmsg);  ## less common
...
=head1 Description
  ...
  this is *not* dependent on any backend model code.
...
sub msghash2string( $msgptr, $msgurlback, $listofread=undef, $tzformat=undef ) {
  ...
  my $msgstring= '<div class="msgarea"> <dl class="dl-horizontal">';
  foreach (@$msgptr) {
...
EOS
    ...
    my $epoch= (defined($tzformat)) ? epochfour($_->{time}, $tzformat) : epochtwo($_->{time});
    $msgstring .= <<EOM;
...
################################################################
## writes a nice version of how much time is left
...
################################################################
## we use global variables because we cannot redirect deep in
## the code.  so we set these global variables and return undef

our $global_redirecturl;
our $global_message;
...
################################################################
## standard starts every webpage.  it makes sure that we have
...
sub domain( $c ) {
  ($c->req->url->to_abs->host =~ /localhost/) and return "localhost:3000";
  my @f= split(/\./, $c->req->url->to_abs->host);
  (@f) or return;  ## never
  return $f[$#f].'.'.$f[$#f-1];
}
...
sub drawform {
  ...
  foreach (@{$_[0]}) {
    ...
    my $hmaxlength= ($f{maxlength}) ? "maxlength=\"$f{maxlength}\"" : "";
	...
	<div class="form-group">
	  ...
	  <div class="col-sm-6">$hpublic
		<input class="form-control foo" id="$name" name="$name" $hhtmltype $hmaxlength $hvalue $hplaceholder $hrequired $hreadonly $hpattern />
	  </div>
        </div>
       );
  }
...
sub epochfour( $epoch, $tzi ) {
  ...
  my $gmtadd= (defined($tzi)) ? " GMT ".(-1)*$tzi.":00" : "";
  ...
  return
    ...
    "<tr> <td>Epoch: </td> <td> <span class=\"epoch\">$epoch</span></td> </tr>\n".
...
sub epochtwo( $epoch ) { qq(<span class="epoch">$epoch</span> ).timedelta($epoch); }


################################################################
## utility functions for bootstrap buttons
...
sub btnblock($url, $text, $belowtext="", $btntypes="btn-default", $textisshort=0) {
  if ($btntypes =~ 1) { $btntypes="btn-default", $textisshort=1; }
  my @w= ( 1, 2, 4, 4 );
  ($textisshort) and @w = ( 2, 2, 4, 4 );
  foreach (@w) { $_ = 12/$_; }
  ...
  return qq(<div class="col-xs-$w[0] col-sm-$w[1] col-md-$w[2] col-lg-$w[3]">).
    btn($url, "<h2>$text</h2>", "btn btn-block $btntypes")
    .$belowtext
    .'</div>';
}
...
sub drawmore($centertype, $actionchoices, $detail, $tzi) {
  ...
  my $changedtime= epochfour( $detail->{mtime}||0, $tzi );
  ...
  my $duetimefour= epochfour( $detail->{duetime}||0, $tzi );
  ...
  <table class="table">
	...
	<tr> <th> visible until </th> <td> $duetimefour
			...
			User Time: <input type="date" id="duedate" placeholder="dd/mm/yyyy" name="duedate" value="$dueyyyymmdd" onblur="submit();" />
	...
	<tr> <td colspan="3"> $backbutton </td> </tr>
    </tbody>
  </table>
EOT
}
